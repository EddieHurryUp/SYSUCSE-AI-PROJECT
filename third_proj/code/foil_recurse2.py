...
def find_base(names , rules_set , target , back_ground , negative_examples , positive_examples):
    for it in rules_set:
        result = []
...
def solve(names , rules_set , target , back_ground , negative_examples , positive_examples):
    ...
    while(i < 10):
        ...
        for it in rules_set:
            ...
            if gain > old_gain:
                if i == 1 and it[0] == target:
                    continue
                if i == 2  and (it[0] != target or it[2] != 'y'):
                    continue
                new_rule = it
                old_gain = gain
                saved_m11 = m11
...
def main():
    ...
    rules_set_base = get_background_rules_set(back_ground)
    rules_set_afterbase = get_background_rules_set(back_ground + positive_examples)
    names = get_names(clause)
    base_rule , positive_covered = find_base(names , rules_set_base , target , back_ground , negative_examples , positive_examples)
    # print(positive_covered)
    new_rule_set = []
    new_positive = []
    ...
    # for it in positive_examples:
    #     if it not in positive_covered:
    #         new_positive.append(it)
    # positive_examples = new_positive

    result_recurse = solve(names , rules_set_afterbase , target , back_ground + positive_examples , negative_examples , positive_examples)
    print("===============================================")
    print( ans_process([base_rule] , target) ,";", ans_process(result_recurse , target))
    print("===============================================")
